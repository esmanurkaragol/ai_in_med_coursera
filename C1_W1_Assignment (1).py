...
def check_for_leakage(df1, df2, patient_col):
    ...
    # Birinci veri kümesindeki benzersiz hasta kimliklerini kümeye dönüştür.
    df1_patients_unique = set(df1[patient_col].unique())

    # İkinci veri kümesindeki benzersiz hasta kimliklerini kümeye dönüştür.
    df2_patients_unique = set(df2[patient_col].unique())

    # Her iki veri kümesinde de bulunan hasta kimliklerini bul.
    patients_in_both_groups = df1_patients_unique.intersection(
        df2_patients_unique
    )

    # Ortak kümede en az bir hasta varsa veri sızıntısı vardır.
...
def compute_class_freqs(labels):
    ...
    Args:
        labels (np.array): matrix of labels,
                           size (num_examples, num_classes)

    Returns:
        positive_frequencies (np.array):
            array of positive frequencies for each class,
            size (num_classes)

        negative_frequencies (np.array):
            array of negative frequencies for each class,
            size (num_classes)
    """

    ### START CODE HERE ###

    # Toplam hasta sayısı, yani satır sayısı
    N = labels.shape[0]

    # Her sütundaki 1 değerlerini topla ve toplam hasta sayısına böl
    positive_frequencies = np.sum(labels, axis=0) / N

    # Her sütundaki 0 değerlerini hesapla ve toplam hasta sayısına böl
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    """
    Pozitif ve negatif sınıf ağırlıklarına göre ağırlıklı kayıp
    fonksiyonu oluşturur.

    Args:
        pos_weights (np.array):
            Her sınıf için pozitif örnek ağırlıkları.

        neg_weights (np.array):
            Her sınıf için negatif örnek ağırlıkları.

        epsilon (float):
            Logaritma işleminde sayısal hata oluşmasını önleyen
            küçük değer.

    Returns:
        weighted_loss (function):
            Keras tarafından kullanılacak özel kayıp fonksiyonu.
    """

    def weighted_loss(y_true, y_pred):
        """
        Bütün sınıflardaki ortalama ağırlıklı kayıpları toplar.

        Args:
            y_true (Tensor):
                Gerçek etiketler, boyut:
                (num_examples, num_classes)

            y_pred (Tensor):
                Model tahminleri, boyut:
                (num_examples, num_classes)

        Returns:
            loss:
                Bütün sınıflar üzerinden toplanan skaler kayıp.
        """

        # Toplam kaybı sıfırdan başlat.
        loss = 0.0
        ...
        for i in range(len(pos_weights)):
            ...
            # Her sınıf için ağırlıklı ikili çapraz entropiyi hesapla
            # ve sınıfın ortalama kaybını toplam kayba ekle.
            loss += -K.mean(
                pos_weights[i]
                * y_true[:, i]
