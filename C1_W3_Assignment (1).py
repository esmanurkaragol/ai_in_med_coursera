...
def get_sub_volume(image, label, 
    ...
    while tries < max_tries:
        ...
        # Rastgele başlangıç koordinatları seç
        start_x = np.random.randint(0, orig_x - output_x + 1)
        start_y = np.random.randint(0, orig_y - output_y + 1)
        start_z = np.random.randint(0, orig_z - output_z + 1)

        # İlgili label patch'ini çıkar
        y = label[
        ...
        # Background oranı uygunsa patch'i kabul et
        if bgrd_ratio < background_threshold:

            # Görüntü patch'ini kopyala
            X = np.copy(
                image[
                    start_x:start_x + output_x,
                    start_y:start_y + output_y,
...
def standardize(image):
    ...
    Returns:
        standardized_image (np.array):
            standardized version of input image
    """

    ### START CODE HERE ###

    # Görüntü ile aynı boyutta sıfırlardan oluşan dizi oluştur
    standardized_image = np.zeros(image.shape)

    # Kanallar üzerinde dolaş
    for c in range(image.shape[0]):

        # Z dilimleri üzerinde dolaş
        for z in range(image.shape[3]):

            # İlgili kanal ve z dilimini al
            image_slice = image[c, :, :, z]

            # Ortalamayı çıkar
            centered = image_slice - np.mean(image_slice)

            # Standart sapma sıfır değilse ölçekle
            ...
            # Standardize edilmiş dilimi yerine yaz
...
def dice_coefficient(y_true, y_pred, axis=(1, 2, 3), 
    ...
    dice_numerator = (
        2 * K.sum(y_true * y_pred, axis=axis)
        + epsilon
    )
...
def compute_class_sens_spec(pred, label, class_num):
    ...
    # True Positive
    tp = np.sum(
        (class_pred == 1) & (class_label == 1)
    )

    # True Negative
    tn = np.sum(
        (class_pred == 0) & (class_label == 0)
    )

    # False Positive
    fp = np.sum(
        (class_pred == 1) & (class_label == 0)
    )

    # False Negative
    fn = np.sum(
        (class_pred == 0) & (class_label == 1)
    )
